  ...
  useEffect(() => {
    if (id) {
      api.get(`/api/alunos/${id}/`)
        ...
        .catch((err) => alert(`Erro ao buscar aluno: ${err}`));
    }
  }, [id]);
  ...
  const handleSubmit = (e) => {
    ...
    if (validate()) {
      ...
      api[method](url, formData)
        .then(() => navigate('/alunos')) // Redireciona após salvar
        .catch((err) => alert(`Erro ao salvar aluno: ${err}`));
    }
  };
    ...
    <>
      ...
      <Box component="form" onSubmit={handleSubmit} noValidate>
        ...
        <Button
          ...
          sx={{ marginTop: 2 }}
        >
          {id ? 'Atualizar Aluno' : 'Criar Aluno'}
        </Button>
      </Box>
        </Paper>
    </>
